...
else if (true)
{
  ...
  if (_ws.gateKeeper.getString("TransferReason").equalsIgnoreCase("RASClosed"))
  {
    ...
    else if (true)
    {
      ...
      else if (true)
      {
        ...
        else if (true)
        {
          ...
          if (_ws.gateKeeper.getString("TransferReason").equalsIgnoreCase("BloodScreening"))
          {
        	  _ws.pl.addPromptGroupAudio('messageid', 'RP9050_i_1');  /* Just a moment while I transfer you. */
          }
          else if (true)
          {
        	// IFCONDITION: Else if TransferReason = Accuchek
              if (_ws.gateKeeper.getString("TransferReason").equalsIgnoreCase("Dispatch"))
              {
            	  _ws.pl.addPromptGroupAudio('messageid', 'RP9050_i_1');  /* Just a moment while I transfer you. */
              }
              else if (true)
              {
            	// IFCONDITION: Else if TransferReason = Accuchek
                  if (_ws.gateKeeper.getString("TransferReason").equalsIgnoreCase("Parts"))
                  {
                	  _ws.pl.addPromptGroupAudio('messageid', 'RP9050_i_1');  /* Just a moment while I transfer you. */
                  }
                  else if (true)
                  {
                	// IFCONDITION: Else if TransferReason = Accuchek
                      if (_ws.gateKeeper.getString("TransferReason").equalsIgnoreCase("ProductOrder"))
                      {
                    	  _ws.pl.addPromptGroupAudio('messageid', 'RP9050_i_1');  /* Just a moment while I transfer you. */
                      }
                      else if (true)
